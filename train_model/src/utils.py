...
from src.dataset import load_manifest_data
...
def output_root(cfg: dict) -> Path:
    return Path(cfg["outputs"]["root"])


def ensure_output_dirs(cfg: dict) -> None:
    root = output_root(cfg)
    for rel in ["runs", "reports", "figures", "tables"]:
...
def compute_mean_std(
    ...
    std = np.maximum(flattened.std(axis=0), 1e-6)
    return mean.astype(np.float32), std.astype(np.float32)


def evaluate_torch(model: torch.nn.Module, loader, device: torch.device) -> tuple[float, float, np.ndarray, np.ndarray]:
    model.eval()
    all_preds = []
    all_labels = []
    with torch.no_grad():
...
def inference_time_ms(model: torch.nn.Module, loader, device: torch.device, warmup: int = 3) -> float:
    ...
    return 0.0 if total_samples == 0 else (total_time / total_samples) * 1000.0
